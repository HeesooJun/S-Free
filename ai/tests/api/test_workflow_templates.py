...
def test_instantiate_workflow_template_returns_root_and_child_mapping(client):
    ...
    template = WorkflowTemplate(
        ...
        owner_user_id=None,
        session_id=session_id,
        name=" 관련 조사 공유 및 UX 화면 확인",
        description=" 관련 조사 결과를 Mattermost 우리만 채널로 보내고, UX 디자이너가 만든 화면을 확인한 뒤 사용자에게 보여준다.",
        graph=WorkflowTemplateGraph(
            nodes=[
                WorkflowTemplateNode(
                    slot_key="research",
                    title=" 관련 조사",
                    description=" 관련 최신 동향을 조사한다.",
                ...
                WorkflowTemplateNode(
                    slot_key="screen",
                    title="조사 기반 화면 작성",
                    description=" 관련 조사 결과를 바탕으로 화면을 만든다.",
