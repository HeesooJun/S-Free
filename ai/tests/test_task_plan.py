...
def test_agent_loop_runtime_step_uses_user_facing_work_title_before_internal_title():
    ...
    task = planner.materialize_task(
        ...
        input_payload={
            "prompt": " 관련 조사 결과를 우리만 채널로 보내줘.",
            "workContext": {"title": " 조사 공유 및 UX 화면 확인"},
        },
        handler=handler,
    )
    ...
    assert task.title == " 조사 공유 및 UX 화면 확인"
    assert step.title == " 조사 공유 및 UX 화면 확인"
    assert step.summary_message == " 조사 공유 및 UX 화면 확인 중"
    assert step.detail_json["semanticDetail"]["semanticStep"] == " 조사 공유 및 UX 화면 확인"
